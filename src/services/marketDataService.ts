...
export async function getHistoricalMarketData(ticker: string, range: TimeRange): Promise<MarketDataResult> {
  const now = new Date();
  let period1: Date;
  ...
  try {
    ...
    const result = await yahooFinance.chart(queryTicker, queryOptions) as any;
    
    if (!result || !result.quotes || result.quotes.length === 0) {
      throw new Error(`No historical data found for ${ticker}`);
    }

    const history: MarketDataPoint[] = result.quotes

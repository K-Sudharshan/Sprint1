import { MarketSnapshot, UserProfile, SessionLog, AgentTelemetry } from './types/models';
...
async function runWithTelemetry<T>(
  ...
  displayName: string,
  agentFn: Promise<T>
): Promise<{ result: T | null; telemetry: AgentTelemetry }> {
...
export async function runSession(ticker: string, userProfile: UserProfile): Promise<SessionLog> {
  ...
  const [marketResult, sentimentResult, ragResult] = await Promise.all([
    runWithTelemetry(
      'market_signal_agent',
      'Market Signal',
      runMarketSignalAgent(snapshot)
    ),
    runWithTelemetry(
      'sentiment_intelligence_agent',
      'Sentiment',
      runSentimentAgent(snapshot)
    ),
    runWithTelemetry(
      'fundamental_rag_agent',
      'Fundamental RAG',
      runFundamentalRagAgent(ticker, 'earnings and regulatory reviews')
    )
  ]);
...
async function main() {
  ...
  const diffAggressive = await runCounterfactual(
  ...
  const diffConservative = await runCounterfactual(
    log.agentOutputs.marketSignal,
    log.agentOutputs.sentiment,
    log.agentOutputs.fundamentalRag,
    profile,

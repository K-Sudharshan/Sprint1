export async function runMarketSignalAgent(snapshot) {
    ...
    let volumeScore = 0.5;
    if (zScore > 2) {
        volumeLabel = 'High Anomaly';
        volumeScore = 0.9;
    }
    else if (zScore > 1) {
        volumeLabel = 'Elevated';
        volumeScore = 0.7;
    }
    else if (zScore < -1) {
        volumeLabel = 'Low';
    ...
    else if (snapshot.indicators.rsi > 50) {
        trendLabel = 'Upward';
        trendScore = 0.6;
    }
    else {

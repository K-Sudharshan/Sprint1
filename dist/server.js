...
app.post('/api/counterfactual', async (req, res) => {
    try {
        ...
        const diff = await runCounterfactual(log.agentOutputs.marketSignal, log.agentOutputs.sentiment, log.agentOutputs.fundamentalRag, log.userProfile, targetProfile);
        res.json(diff);
    }
    catch (error) {
...
import { getHistoricalMarketData } from './services/marketDataService';
app.get('/api/market-data', async (req, res) => {
    try {
        ...
        const timeRange = validRanges.includes(range) ? range : '1M';
